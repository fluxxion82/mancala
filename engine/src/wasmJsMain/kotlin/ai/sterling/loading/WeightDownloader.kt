...
/**
 * Path (relative to the page) at which the `ai.sterling:mancala` UI module's Compose
 * Resources place the gzipped weights. On the website this resolves to
 * `https://sterlingalbury.com/composeResources/ai.sterling.mancala.resources/files/mancala_weights.bin.gz`.
 */
const val MANCALA_WEIGHTS_GZ_PATH: String =
...
/**
 * Browser weight loader: IndexedDB cache lookup by [cacheKey], otherwise `fetch(url)`
 * of the gzipped weights with progress, native `DecompressionStream('gzip')`, then a
 * best-effort IndexedDB write. Drives [onState] through the loading phases and ends
 * on [WeightLoadingState.Initializing]; throws on network / decompression failure.
 *
 * Returns the uncompressed weight bytes ready for [ai.sterling.createAiBackend] /
 * `NeuralNetEngine.create`. Compose-free, so any wasmJs front end can use it.
 */
suspend fun downloadWeightBytes(
    url: String = MANCALA_WEIGHTS_GZ_PATH,
    cacheKey: String,
    onState: (WeightLoadingState) -> Unit = {},
): ByteArray {
    onState(WeightLoadingState.Checking)
    runCatching { idbLookup(cacheKey) }
        .getOrNull()
        ?.let {
    ...
    val gz = fetchWithProgress(url) { received, total ->
    ...
    saveToIdbBestEffort(cacheKey, raw)
